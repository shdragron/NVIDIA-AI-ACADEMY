...
class ROIPoolingCls():
    def __init__(self, mode = 'tf', pool_size=(7,7)):
        # tf : (height , width, channels) 텐서플로우
        # th : (channels, height, width)  파이토치
    ...
    def get_region(self, feature_map, roi_one_boxes): # 특징맵 위 roi 영역 획득
        ...
        if self.mode == 'tf':  # 텐서플로우
            # (height, width, channels)
            # (height, width, channels) ==> 로 차원축소한 feature_map을 전체 roi 크기인 ymin:ymax, xmin:xmax 범위 만큼 슬라이싱
            region_slicing = np.squeeze(feature_map)[ymin:ymax, xmin:xmax, :]
            #print('region slicing shape : ', region_slicing.shape)
        elif self.mode == 'th': # 파이토치
            region_slicing = np.squeeze(feature_map)[:, ymin:ymax, xmin:xmax]
