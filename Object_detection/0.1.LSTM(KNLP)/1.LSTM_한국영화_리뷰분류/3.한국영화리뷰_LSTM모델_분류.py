...
model = Sequential()
model.add(Embedding(word_size, embedding_dim, input_length=30))
model.add(LSTM(hidden_units))
model.add(Dense(1, activation='sigmoid'))
model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])
model.summary()
EarlyStop_cb = EarlyStopping(monitor='loss', verbose=1, patience=4,restore_best_weights=True)
ModelCheck_cb = ModelCheckpoint('best_LSTM_model.h5', monitor='loss',
                                verbose=1, save_best_only=True)

history = model.fit(X_train_pades, y_train, epochs=15,
                    callbacks=[EarlyStop_cb, ModelCheck_cb],batch_size=64)
